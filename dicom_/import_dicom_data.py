...
def get_dicom_store(
    project_id, location, dataset_id, dicom_store_id, is_import=False, link=None
):
    ...
    if is_import:
        data = dicom_stores.import_(
            name=dicom_store_name,
            body={
                "gcsSource": {"uri": f"{link}"},
            },
        ).execute()

    print(json.dumps(dicom_store, indent=2))

    return dicom_store, data
...
get_dicom_store(
    ...
    is_import=True,
    link="gs://gcs-public-data--healthcare-nih-chest-xray/dicom/*.dcm",
)
